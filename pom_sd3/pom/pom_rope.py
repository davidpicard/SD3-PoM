...
class PoMRoPE(PoM):
    ...
    *before* the element-wise gating:
    ...
    def __init__(
        ...
        # Per-channel learnable scale applied to h_proj before pom_activation.
        # Log-uniform init across [0.01, 5] gives equal coverage per decade:
        # ~statistics mode (small scale) through ~detection mode (large scale).
        log_scale = torch.empty(rope_dim).uniform_(math.log(0.01), math.log(5.0))
        self.h_scale = nn.Parameter(log_scale.exp())
    ...
    def forward(
        ...
        h_proj = h_proj * self.h_scale  # per-channel activation scale
